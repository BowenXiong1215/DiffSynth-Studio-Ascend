...
def attention_forward(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, q_pattern="b n s d", k_pattern="b n s d", v_pattern="b n s d", out_pattern="b n s d", dims=None, attn_mask=None, scale=None, is_causal=False, compatibility_mode=False, window_size=None, use_flex=False, score_mod=None):
    # CUDA attention packages can be importable on mixed development hosts but
    # cannot consume NPU tensors. PyTorch SDPA is the correctness baseline for
    # Ascend; an optimized NPU kernel can be selected after numerical alignment.
    if q.device.type != "cuda" and ATTENTION_IMPLEMENTATION not in ("torch",):
