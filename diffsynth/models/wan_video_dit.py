...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False):
    # flash-attn, SageAttention and their kernels are CUDA implementations.
    # Never select them for an NPU tensor even if a package is importable.
    compatibility_mode = compatibility_mode or q.device.type != "cuda"
...
def sinusoidal_embedding_1d(dim, position):
    # Ascend does not provide the same float64 operator coverage as CPU/CUDA.
    # Float32 is sufficiently precise for timestep embeddings and avoids a
    # device fallback in the hot path.
    compute_dtype = torch.float32 if position.device.type == "npu" else torch.float64
    sinusoid = torch.outer(position.to(compute_dtype), torch.pow(
        10000, -torch.arange(dim//2, dtype=compute_dtype, device=position.device).div(dim//2)))
