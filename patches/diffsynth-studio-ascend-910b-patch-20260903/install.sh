...
require_source_tree() {
  test -f "${TARGET_ROOT}/pyproject.toml" || {
    echo "Not a DiffSynth-Studio source tree: ${TARGET_ROOT}" >&2
    exit 2
  }
  grep -q 'name = "diffsynth"' "${TARGET_ROOT}/pyproject.toml" || {
...
validate_target() {
  ...
  while IFS=$'\t' read -r upstream expected relative; do
    ...
    if test "${actual}" != "${upstream}" && test "${actual}" != "${expected}"; then
      ...
      echo "Expected upstream DiffSynth-Studio commit 4dbf980d4d0eb34eda136300dd0d72014cff8965" >&2
      exit 4
    fi
  done < "${PATCH_ROOT}/modified.tsv"
...
replace_with_sed() {
  ...
  temporary="/tmp/diffsynth-ascend-replacement.$$.tmp"
...
verify_installed_tree() {
  local upstream expected relative target actual failed=0
  while IFS=$'\t' read -r upstream expected relative; do
    target="${TARGET_ROOT}/${relative}"
    actual="$(sha256_file "${target}")"
...
echo "DiffSynth-Studio Ascend 910B patch: PASS"
